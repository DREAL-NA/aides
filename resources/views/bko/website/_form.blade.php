<form action="{{ $options['url'] }}" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <textarea class="form-control" rows="3" name="themes"
    ...
    <div class="form-group">
        ...
        <div class="input-group">
            ...
            <span class="input-group-btn">
				<button class="btn btn-default" type="button" data-toggle="modal" data-target="#modalNewPerimeter"><i
    ...
    <div class="form-group">
        ...
        <textarea class="form-control" rows="3" name="allocated_budget"
                  id="allocated_budget">{{ old('allocated_budget', $website->allocated_budget) }}</textarea>
    </div>
    <div class="form-group">
        <label for="beneficiaries">Bénéficiaires</label>
        <textarea class="form-control" rows="3" name="beneficiaries"
                  id="beneficiaries">{{ old('beneficiaries', $website->beneficiaries) }}</textarea>
    </div>
    <div class="form-group">
        <label for="website_url">Adresse internet</label>
        <textarea class="form-control" rows="3" name="website_url"
                  id="website_url">{{ old('website_url', $website->website_url) }}</textarea>
    </div>
    <div class="form-group">
        <label for="description">Observations</label>
        <textarea class="form-control" rows="3" name="description"
                  id="description">{{ old('description', $website->description) }}</textarea>
    </div>
    <div class="form-group">
